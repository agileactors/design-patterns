...
@Getter
@Slf4j
public class PrewarTruck implements Truck {
  ...
  public void loadCargo(Double quantity) {
    if (quantity.compareTo(MAX_CARGO_LOAD) > 0) {
      log.info("Loaded cargo with quantity {} into several prewar trucks.", quantity);
    } else {
      log.info("Loaded cargo with quantity {} into one prewar truck.", quantity);
    }
  }
